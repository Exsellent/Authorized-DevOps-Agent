...
class RisksAgent(MCPAgent):
    ...
    Analyses planned features / goals and returns a structured risk profile
    before any code is written. Feeds into Orchestrator's PR creation decision
    and Digest Agent's executive summary.
    """

    # Valid category values the LLM is allowed to return
    ...
    def __init__(self):
        ...
        logger.info("RisksAgent initialised — proactive planning-phase risk assessment")
    ...
    def _get_baseline_risks(
        self, feature: str, issue_number: Optional[int] = None
    ) -> List[RiskItem]:
        """
        ...
        if any(k in lower for k in [
            ...
            risks.append(RiskItem(
                ...
                mitigation_strategy=(
                    "Conduct security design review; use secret scanning (GitHub Advanced Security); "
                    "never commit credentials; prefer short-lived tokens via Auth0 Token Vault."
                ),
                priority=1,
        ...
        if any(k in lower for k in [
            ...
            risks.append(RiskItem(
                ...
                likelihood="medium",
                description=(
                    "Modifying GitHub Actions workflows can introduce supply-chain risks "
                    "or expose secrets to untrusted code."
                ),
        ...
        if any(k in lower for k in [
            ...
            risks.append(RiskItem(
                ...
                severity=RiskSeverity.MEDIUM.value,
                likelihood="medium",
                description="External integrations introduce dependency and availability risks.",
    ...
    def _calculate_overall_risk(self, risks: List[RiskItem]) -> str:
        ...
        if avg >= 3.5: return RiskSeverity.CRITICAL.value
        if avg >= 2.5: return RiskSeverity.HIGH.value
        if avg >= 1.5: return RiskSeverity.MEDIUM.value
        return RiskSeverity.LOW.value

    def _issues_found_list(self, risks: List[RiskItem]) -> List[Dict]:
        """
        Flat list of issues consumed by Orchestrator:
          risks_result.get("issues_found", [])

        Each entry has: title, severity, mitigation_strategy.
    ...
    async def analyze_risks(
        ...
        classification: Optional[Dict] = None, # NEW — from Planner Agent
        file_tree:    Optional[List[str]] = None,  # NEW — from Orchestrator
        repo:         Optional[str]  = None,   # NEW — "owner/repo"
    ) -> Dict[str, Any]:
        """
        Full proactive risk assessment pipeline.
...
"""
        ...
        try:
            ...
            if self._is_invalid_response(llm_text):
                ...
                #    falling back to baseline (original code always called
                ...
                if parsed:
                    ...
                    # LLM responded but JSON was unparseable → hybrid
                    baseline       = self._get_baseline_risks(feature, issue_number)
                    detected_risks = baseline
        ...
        critical_count = sum(1 for r in detected_risks if r.severity == RiskSeverity.CRITICAL.value)
        high_count     = sum(1 for r in detected_risks if r.severity == RiskSeverity.HIGH.value)
        medium_count   = sum(1 for r in detected_risks if r.severity == RiskSeverity.MEDIUM.value)
        low_count      = sum(1 for r in detected_risks if r.severity == RiskSeverity.LOW.value)

        top_concerns          = [r.title for r in detected_risks[:3]]
        mitigation_priorities = [r.mitigation_strategy for r in detected_risks[:3]]

        if overall_risk == RiskSeverity.CRITICAL.value:
            go_no_go = "NO-GO: Requires risk mitigation and architecture review before proceeding"
        elif overall_risk == RiskSeverity.HIGH.value:
            go_no_go = "CONDITIONAL: Proceed with senior oversight and a documented mitigation plan"
        else:
        ...
        executive_summary = ExecutiveRiskSummary(
            ...
            confidence_level="high" if analysis_mode == RiskAnalysisMode.LLM else "medium",
            timestamp=datetime.now().isoformat(),
        )
        ...
        # FIX: plain %s args — no logger.extra= dict
        logger.info(
            "Risk assessment done — feature=%s overall=%s priority=%s mode=%s",
            feature[:50], overall_risk, priority, analysis_mode.value,
        )
        ...
        return {
            ...
            "overall_risk_level": overall_risk,          # overall risk string
            "priority": priority,                        # P0–P3

            # issues_found: flat list consumed by Orchestrator and Code Execution
            # orchestrator.py: risks_result.get("issues_found", [])
            "issues_found": self._issues_found_list(detected_risks),

            # ── Full risk detail ──────────────────────────────────────────────
            "risks": [asdict(r) for r in detected_risks],

            # ── Executive summary — consumed by Digest Agent ──────────────────
            ...
            "metadata": {
                ...
                "analysis_mode": analysis_mode.value,
                "auth0_token_vault": False,   # explicit: Risks does NOT use Token Vault
                "timestamp":     datetime.now().isoformat(),
            },
        }

    # ── MCP Tool: assess_feature_risk (lightweight) ───────────────────────────
    ...
    async def assess_feature_risk(
        ...
        self._next_step(
            ...
            output_data={"overall_risk": overall_risk, "risks_count": len(risks)},
        )
...
def get_agent_status() -> Dict[str, Any]:
    return {
        "agent_name":  "risks",
        "status":      "HEALTHY",
        "capabilities": ["analyze_risks", "assess_feature_risk"],
        "features": [
        ...
        "llm_powered": True,
        "focus":       "Pre-implementation planning risks",
        "timestamp":   datetime.now().isoformat(),
    }